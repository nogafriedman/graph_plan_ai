    ...
    def get_successors(self, state):
        ...
        for action in self.actions:
            ...
            if action.all_preconds_in_list(state):
                new_state = frozenset(state.difference(set(action.get_delete())).union(set(action.get_add())))
...
def max_level(state, planning_problem):
    ...
    while not planning_problem.is_goal_state(graph[-1].get_proposition_layer().get_propositions()):
        ...
        pg_next.expand_without_mutex(graph[-1])
...
def level_sum(state, planning_problem):
    ...
    for proposition in state:
        initial_prop_layer.add_proposition(proposition)

    initial_graph = PlanGraphLevel()
    initial_graph.set_proposition_layer(initial_prop_layer)
    graph_levels = [initial_graph]
    level_sum, cur_level = 0, 0

    cur_props = initial_graph.get_proposition_layer().get_propositions()
    remaining_goals = set(planning_problem.goal)
    for p in cur_props:
        remaining_goals.discard(p)

    while planning_problem.goal_state_not_in_prop_layer(cur_props):
        new_graph = PlanGraphLevel()
        new_graph.expand_without_mutex(graph_levels[cur_level])
        graph_levels.append(new_graph)
        cur_level += 1

        if is_fixed(graph_levels, cur_level):
            return float('inf')

        cur_props = new_graph.get_proposition_layer().get_propositions()
        for proposition in cur_props:
            if proposition in remaining_goals:
                remaining_goals.discard(proposition)
                level_sum += cur_level
