...
class GraphPlan(object):
    ...
    def graph_plan(self):
        ...
        # for action in plan_solution:  # todo: remove
...
def independent_pair(a1, a2):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Check for inconsistent effects (one action adds a proposition that the other action deletes)
    for prop in a1.get_add():
        if a2.is_neg_effect(prop):
            return False
    for prop in a2.get_add():
        if a1.is_neg_effect(prop):
            return False

    # Check for interference (one action deletes the precondition of another)
    ...
    # If we've passed all checks, the actions are independent
    return True
